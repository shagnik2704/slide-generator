...
def generate_draft_outline(outline_data: Dict) -> str:
    ...
    # Get platform label based on type
    if outline_type == 'ICT':
        platform_label = "ICT Platform/Program"
    else:
        platform_label = "Platform Name"
    
    draft = f"""# Course Outline Draft ({outline_type})

## Tutorial Information
- **Course Outline Name:** {outline_data.get('outline_name', 'N/A')}
- **{platform_label}:** {outline_data.get('platform_name', 'N/A')}
- **OS Version:** {outline_data.get('os_version', 'N/A')}
...
"""
    ...
    draft += f"""
## Examples
- **{'Teaching Scenarios/Examples (core use case)' if outline_type == 'ICT' else 'Core Example'}:** {outline_data.get('core_example', 'N/A')}
- **{'Allied examples/scenarios' if outline_type == 'ICT' else 'Allied Examples'}:** {'; '.join(outline_data.get('allied_examples', [])) or 'None'}
