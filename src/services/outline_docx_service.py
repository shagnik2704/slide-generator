...
def create_outline_docx(outline_data: dict, output_path: str = None) -> str:
    ...
    # === ABOUT THE COURSE ===
    doc.add_heading('About the course (Brief introduction)', level=1)
    doc.add_paragraph(outline_data.get('about_course', ''))
    ...
    # Core example
    examples_table.rows[0].cells[0].text = "Core example used in the series"
    ...
    for tutorial in outline_data.get('tutorial_rows', []):
        ...
        tutorial_table = doc.add_table(rows=num_rows, cols=3)
        tutorial_table.style = 'Table Grid'
        
        # Header row
        header_row = tutorial_table.rows[0]
        headers = ["Topics Details", "Time (secs)", "Comments"]
