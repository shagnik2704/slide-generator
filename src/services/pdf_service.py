...
def create_script_pdf(json_data, output_filename=None):
    """Generate a Spoken Tutorial script PDF matching the standard format."""
    # Get project root (3 levels up from src/services/pdf_service.py)
    project_root = Path(__file__).parent.parent.parent
    static_dir = project_root / "static"
    static_dir.mkdir(exist_ok=True)
    
    if output_filename is None:
        output_filename = static_dir / "script_review.pdf"
    else:
        # If relative path provided, make it relative to static dir
        if not Path(output_filename).is_absolute():
            output_filename = static_dir / output_filename
        else:
